...
gff    <- fread(file="GRCm38.p4.Refseq.gff", skip = 9, stringsAsFactors = F, header = F, fill = T, na.strings = c("", "NA"), sep="\t") %>% na.omit() #deals with unwanted #comment lines in the gff
...
for(name in names(parents.tree)) {
    ...
    if(nrow(slice) == 1){
      ...
      gff[name, on = 'ID', V5 := gene_end]
        ...
        # con <- file("GRCm38.p4.Refseq.gff", "r")
        # header <- readLines(con, n = 7)
        # write.table(header, file = "GRCm38.p4.Refseq.fixed.gff", col.names = F, row.names = F, quote = F)
        # write.table(gff[,1:9], file = "GRCm38.p4.Refseq.fixed.gff", sep = "\t", row.names = F, col.names = F, quote = F, append = T)
        # close(con); rm(con)
...
gff2 <- remove.features(gff, c('cDNA_match','match','primary_transcript','region','rRNA','tRNA'), parents.table)
        con <- file("GRCm38.p4.Refseq.gff", "r")
        header <- readLines(con, n = 7)
        write.table(header, file = "GRCm38.p4.Refseq.coding.gff", col.names = F, row.names = F, quote = F)
        write.table(gff2[,1:9], file = "GRCm38.p4.Refseq.coding.gff", sep = "\t", row.names = F, col.names = F, quote = F, append = T)
        close(con); rm(con)
